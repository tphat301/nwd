<?php
    ...
    $service = $cache->get("select name$lang, slugvi, slugen, desc$lang, content$lang, date_created, id, photo from #_news where type = ? and find_in_set('noibat',status) and find_in_set('hienthi',status) order by id desc", array('dich-vu'), 'result', 7200);
    ...
    $votes = $cache->get("select name$lang, desc$lang, photo from #_news where type = ? and find_in_set('hienthi',status) order by numb asc", array('cam-nhan-khach-hang'), 'result', 7200);
